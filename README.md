...
A. Architecture & Search Pipeline
Instead of running brute-force linear scans across high-dimensional space, this system uses two synchronized self-balancing trees to prune search candidates:
                         [Query Vector q]
                                 │
                     Compute norm: nq = ||q||
...
Root Selection Heuristic: Dynamically elects the vector closest to the global average distance (d_bar) as the root, optimizing tree topology for bounded range searches.
